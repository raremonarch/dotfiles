...
pipewire &
pipewire-pulse &
# wireplumber exits if pipewire's socket isn't up yet, so wait for it (max 5s)
(for i in $(seq 50); do [ -S "$XDG_RUNTIME_DIR/pipewire-0" ] && break; sleep 0.1; done; exec wireplumber) &
